...
const Articles = () => {
  ...
  // 添加数据源切换状态 - 默认使用已分析新闻
  const [dataSource, setDataSource] = useState<'currentnews' | 'analyzed'>('analyzed');
  ...
  useEffect(() => {
    const fetchNewsData = async () => {
      ...
      try {
        ...
        console.log('当前数据源:', dataSource, '当前页码:', currentPage);
        
        if (dataSource === 'analyzed') {
          // 使用新的分页接口获取已分析的新闻
          console.log('调用已分析新闻接口:', `/api/analyzed_news_paginated?page=${currentPage}&page_size=${pageSize}&sort_field=participants&sort_order=desc`);
          response = await fetch(`/api/analyzed_news_paginated?page=${currentPage}&page_size=${pageSize}&sort_field=participants&sort_order=desc`);
          res = await response.json();
          console.log('已分析新闻接口响应:', res);
          ...
          if (res.success) {
            ...
            // 已分析新闻数据需要转换为前端期望的格式
            const transformedAnalyzedData = data.map((item: any) => ({
              id: item.id,
              title: item.title,
              area: item.type,
              description: item.introduction,
              heatValue: Math.floor((item.participants || 0) * 10000), // 转换为显示的热度值
              date: item.analyzed_at ? item.analyzed_at.split('T')[0] : new Date().toISOString().split('T')[0],
              platform: item.platform || '未知',
              location: { x: item.x || 116.4074, y: item.y || 39.9042 },
              emotion: item.emotion,
              spreadSpeed: item.spreadSpeed,
              spreadRange: item.spreadRange,
              participants: item.participants,
              heat_history: item.heatTrend ? item.heatTrend.map((h: any) => ({
                comprehensive_heat: h.value || 0,
                normalized_heat: h.value || 0,
                timestamp: h.date || new Date().toISOString(),
                weighted_heat_value: h.value || 0
              })) : []
            }));
            
            // 对于已分析新闻，直接使用当前页数据，不进行前端分页
            setNewsData(transformedAnalyzedData);
            setFilteredData(transformedAnalyzedData);
            
            // 提取唯一的领域标签
            const uniqueAreaList = transformedAnalyzedData.map((item: any) => item.area).filter((area: any) => typeof area === 'string') as string[];
            setUniqueAreas([...new Set(uniqueAreaList)]);
          } else {
            throw new Error(res.message || '获取分析数据失败');
          }
        } else {
          // 使用原有接口获取当前新闻
          console.log('调用当前热搜接口:', '/api/currentnews');
          response = await fetch('/api/currentnews');
          res = await response.json();
          console.log('当前热搜接口响应:', res);
          data = res.data;
    ...
    fetchNewsData();
  }, [dataSource, currentPage]); // 添加dataSource和currentPage作为依赖
  ...
  const handleResetFilters = () => {
    ...
    setDataSource('analyzed'); // 重置数据源为已分析新闻
    setHeatRange('all'); // 重置热度范围
  ...
  useEffect(() => {
    if (!loading && newsData.length > 0 && dataSource === 'currentnews') {
      // 只对当前热搜数据进行前端筛选，已分析新闻通过后端分页处理
      handleSearchClick();
    }
  }, [sortType, eventLibrary, timeRange, searchText, selectedAreas, loading, newsData, selectedPlatform, heatRange, dataSource]);
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', marginRight: '24px', marginBottom: '8px' }}>
          <span style={{ marginRight: '8px', fontSize: '14px', color: '#666' }}>数据源:</span>
          <Select
            value={dataSource}
          ...
          >
            <Option value="analyzed">已分析新闻</Option>
            <Option value="currentnews">当前热搜</Option>
          </Select>
        </div>
          ...
          {filteredData.length === 0 && !loading ? (
            ...
            <Row justify="center" style={{ marginTop: 32, marginBottom: 32 }}>
              <Pagination
                current={currentPage}
                pageSize={pageSize}
                total={dataSource === 'analyzed' ? totalCount : filteredData.length}
